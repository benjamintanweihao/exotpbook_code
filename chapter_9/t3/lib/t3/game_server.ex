defmodule T3.GameServer do
  ...
  def handle_call({:join, p_id}, _from, State[x_id: nil] = state) do
	  { :reply, :waiting, state.update(x_id: p_id, refs: [ Process.monitor(p_id) ]) }
  end
  ...
  def handle_call({:join, p_id}, _from, State[x_id: x_id, o_id: nil] = state) 
    when is_pid(x_id) do
	  { :reply, :in_progress, state.update(o_id: p_id, 
	                                refs: [ Process.monitor(p_id) | state.refs ]) }  
	end
  ...
  def handle_cast(:stop, state) do
	...
	def handle_info(msg, state) do
	  case msg do
	    {:DOWN, _ref, :process, crashed_pid, _reason} ->
	      [state.x_id, state.o_id] 
	      |> List.delete(crashed_pid) 
	      |> Enum.each fn pid -> 
	           send(pid, :other_player_left) 
	         end
	      { :stop, :other_player_left, state }    
	    _ -> 
	      { :noreply, state } 
	  end
	end
  ...
  def terminate(reason, state) do
    IO.puts "Server terminated due to: #{reason}. State is #{inspect state}"
	  Enum.each state.refs, fn ref -> Process.demonitor(ref) end
